...
# ----------------------------------------------------------------
def load_data(sequence: str, spatial_res: int, full_seq: bool, p):
    ...
    if sequence.split('/')[-1].split('_')[0] == 'LOI':
        LOI =  True
    if extension == 'nii':
        return load_NIFTI_sequence(sequence=sequence, spatial_res=spatial_res, full_seq=full_seq)
    if extension == 'dicom':
...
# ----------------------------------------------------------------
def load_NIFTI_sequence(sequence: str, spatial_res: int, full_seq: bool):
    """ Loads the first frame or the sequence stored in DICOM and returns the interpolated image(s) and the scale coefficient. """

    data = nib.load(sequence)
    sq = data.get_fdata()
    spatial_res_y = data.header.get_best_affine()[0,0] * 1000 / 10
    ...
    if full_seq == True:
        ...
        out, scale = sequence_preprocessing(sequence = sq, current_spatial_res = spatial_res_y, spatial_res = spatial_res)
