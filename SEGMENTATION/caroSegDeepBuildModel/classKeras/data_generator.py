...
# class dataGenerator(tensorflow.keras.utils.Sequence):
#
#     def __init__(self, partitionss, labels, data_augmentation, batch_size=16, dim=(512, 128, 1),  shuffle=True):
#         """ DataGenerator1Channel apply on the fly data augmentation by using imgaug. """
#         self.dim = dim
#         self.batch_size = batch_size
#         self.labels = labels
#         self.shuffle = shuffle
#         self.partitionss = partitionss
#         self.keys = list(partitionss.keys())
#         self.data_augmentation = data_augmentation
#         self.seq = iaa.Sequential([iaa.Fliplr(0.5),
#                                    iaa.Flipud(0.5),
#                                    iaa.Affine(shear=(-2, 2),
...
#         for i, ID in enumerate(list_IDs_temp):
#             # --- Store sample
#             X[i,] = self.partitionss[ID][()].reshape(self.dim)
#             # --- Store class
