...
def setParameters():
  ...
  p = Parameters(
    ...
    PATH_TO_SKIPPED_SEQUENCES='/home/laine/Desktop',               # If an image can't be part of the sets then it saves in .txt file
  ...
  fid = open(os.path.join('package_parameters', 'get_parameters_dataset.py'), 'rt')
  data = fid.read()
  data = data.replace('setParameters()', 'getParameters()')
  fid.close()
  fid = open(os.path.join('package_parameters', 'get_parameters_dataset.py'), 'wt')
  fid.write(data)
  fid.close()
