...
class annotationClass():
    ...
    def __init__(self,
        ...
        if p.MANUAL_FAR_WALL_DETECTION == True and p.AUTOMATIC_METHOD==False:
            ...
            self.initialization(localization=pos,
                                scale=scale)
            ...
            width_window = self.borders['rightBorder']-self.borders['leftBorder']
            if width_window < 128:
                ''' the patch is larger than the window, we have to change the size of the ROI to process the image'''
                middle_position =  self.borders['leftBorder'] + round(width_window/2)
                # TODO: a temporary solution because as could have border problems with it.
                self.bordersROI['leftBorder'] = middle_position - 70
                self.bordersROI['rightBorder'] = middle_position + 70
            else:
                # self.borders['leftBorder'] = 160
                # self.borders['rightBorder'] = 610
    ...
    def updateAnnotation(self,
        ...
        #             self.mapAnnotation[frameID, i, 0] = j + offset
        ...
        #             self.mapAnnotation[frameID, i, 1] = dim[0] - 1 - j + offset
    ...
    def LoadPredictedAnnotationLinearHypothesis(self,
        ...
        '''
        pathToPredictionFW = "../../far_wall_carotide_detection/results/segmentation/"
    ...
    def extendLinearBorder(self):
        '''
        We extend the border if the ROI is to thin
        '''

        # --- we modify the array using linear interpolation
        # left border
        x_a = self.borders['leftBorder']
        y_a = self.mapAnnotation[0, x_a, 0]

        x_b = self.borders['rightBorder']-1
        y_b = self.mapAnnotation[0, x_b, 0]

        slop_L = (y_b-y_a)/(x_b-x_a)

        for k in range(self.bordersROI['leftBorder'], self.borders['leftBorder'], 1):
            if slop_L*(k-x_a) + y_a + 150 < self.sequenceDimension[1] and slop_L*(k-x_a) + y_a >150:
                self.mapAnnotation[0, k, 0] = slop_L*(k-x_a) + y_a
            elif slop_L*(k-x_a) + y_a <= 128:
                self.mapAnnotation[0, k, 0] = 150
        ...
        x_b = self.borders['leftBorder']
        y_b = self.mapAnnotation[0, x_b, 0]

        slop_R = (y_b-y_a)/(x_b-x_a)


        for k in range(self.borders['rightBorder'], self.bordersROI['rightBorder'], 1):
            if slop_R*(k-x_a) + y_a + 150 < self.sequenceDimension[1] and slop_R*(k-x_a) + y_a > 150:
                self.mapAnnotation[0, k, 0] = slop_R*(k-x_a) + y_a
            elif slop_R*(k-x_a) + y_a <= 128:
                self.mapAnnotation[0, k, 0] = 150
    ...
    def LoadPredictedAnnotation(self,
        ...
        pathToPredictionFW = os.path.join(p.PATH_FAR_WALL_DETECTION_RES, 'SEGMENTATION')

        mat_IFC3 = self.loadPrediction(self.patient.split('.')[0] + ".txt", pathToPredictionFW)
        mat_IFC4 = mat_IFC3.copy()

        self.mapAnnotation[0, :, 0] = scale*mat_IFC3
        self.mapAnnotation[0, :, 1] = scale*mat_IFC4

        for k in range(mat_IFC3.shape[0]):
            if mat_IFC3[k]!=0:
                self.borders['rightBorder'] = k
        for k in range(mat_IFC3.shape[0]-1, 1, -1):
            if mat_IFC3[k]!=0:
                self.borders['leftBorder'] = k

        self.bordersROI=self.borders

        # --- we remove annotations that are outside the boundaries
        self.mapAnnotation[0, 0:self.borders['leftBorder']+1, 0] = 0
        self.mapAnnotation[0, self.borders['rightBorder']:, 0] = 0
        self.mapAnnotation[0, 0:self.borders['leftBorder']+1, 1] = 0
        self.mapAnnotation[0, self.borders['rightBorder']:, 1] = 0
        ...
        self.mapAnnotation[0, self.borders['leftBorder'], 0] = self.mapAnnotation[0, self.borders['leftBorder']+1, 0]
        self.mapAnnotation[0, self.borders['leftBorder'], 1] = self.mapAnnotation[0, self.borders['leftBorder'] + 1, 1]

        width_window = self.borders['rightBorder'] - self.borders['leftBorder']
        if width_window < 128:
            ''' the patch is larger than the window, we have to change the size of the ROI to process the image'''
            middle_position = self.borders['leftBorder'] + round(width_window / 2)
            # TODO: a temporary solution because as we could have border problems with it.
            self.bordersROI['leftBorder'] = middle_position - 80
            self.bordersROI['rightBorder'] = middle_position + 80
            self.extendLinearBorder()
        else:
            self.bordersROI = self.borders
    ...
    def FWAutoInitialization(self, img, seed):
        ...
        # --- j cannot exceed limit
        limit = dim[0] - 1
        # --- delineation of the centre line
        for i in range(seed[1]-1, xEnd + 1):
            # --- if condition while a boundary is found
            condition = True
            while condition == True:
