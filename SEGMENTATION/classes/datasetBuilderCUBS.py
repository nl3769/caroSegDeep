...
def save_dic_to_HDF5(dic_datas, path: str):
    ...
    for key in dic_datas.keys():
        ...
        for i in data["masks"].keys():
            tmp_gr_mask.create_dataset(name=i, data=data["masks"][i],
                                       dtype=np.uint8)  # we convert in uint8 order to gain memory usage
...
class datasetBuilderWall():
    ...
    def build_data(self):
        ...
        for data_base in self.p.DATABASE_NAME:
            ...
            for img_index in tqdm(range(len(files_cubs[0:200])), ascii=True, desc='Patch (Mask + image): ' + data_base):
