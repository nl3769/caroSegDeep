...
def bottleneck(x, filters_bottleneck, mode='cascade', depth=6, kernel_size=(3, 3), activation='relu', kernel_regularizer = None, dropout=None):
    ...
    if mode == 'cascade': 
        for i in range(depth):
            x = Conv2D(filters_bottleneck, kernel_size, activation=activation, padding='same', dilation_rate=dil_f[i], kernel_initializer='glorot_uniform', kernel_regularizer = kernel_regularizer)(x)
            x = BatchNormalization()(x)
            dilated_layers.append(x)

    # Like "Atrous Spatial Pyramid Pooling"
    elif mode == 'parallel': 
        for i in range(depth):
            x = Conv2D(filters_bottleneck, kernel_size, activation=activation, padding='same', dilation_rate=2 ** i, kernel_initializer='glorot_uniform', kernel_regularizer = kernel_regularizer)(x)
            dilated_layers.append(x)
...
def custom_dilated_unet(input_shape,
    ...
    dec = decoder(bottle, skip, skip_pool_col, filters, n_block=n_block, kernel_size=kernel_size, SE = SE, n_pool_col=n_pool_col, kernel_regularizer = kernel_regularizer, dropout = dropout)
